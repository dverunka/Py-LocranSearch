...
GRAPH_BOUNDS = {
    ...
    "schweffel": (0, MAX_FES, -419 * DIMENSIONS, 0)
}
...
def main():
    ...
    for cost_function_name, cost_function in COST_FUNCTIONS.items():
        ...
        for search_algorithm_name, search_algorithm in SEARCH_ALGORITHMS.items():
            ...
            # all runs
            init_graph(cost_function_name, f"{search_algorithm_name}, {cost_function_name}, {DIMENSIONS}D, {RUNS} runs")
            cost_best_results = []
            cost_history_results = []

            for _ in range(RUNS):

                cost_best, position_best, costs_history = search_algorithm(BOUNDS[cost_function_name], DIMENSIONS, cost_function, MAX_FES)
                cost_best_results.append(cost_best)
                cost_history_results.append(costs_history)

                # print(f"{search_algorithm_name}, {cost_function_name}, {DIMENSIONS}D: cost_best={cost_best}, position_best={position_best}")

                plt.plot(range(1, len(costs_history) + 1), costs_history, linewidth = 0.5)

            plt.savefig(f"{OUTPUT_DIR}{search_algorithm_name}_{cost_function_name}_all.png")
            plt.clf()
            plt.cla()
            plt.close()
            ...
            init_graph(cost_function_name, f"{search_algorithm_name}, {cost_function_name}, {DIMENSIONS}D, average best run")

            plt.plot(range(1, len(average_run_history) + 1), average_run_history, linewidth=1)
            plt.savefig(f"{OUTPUT_DIR}{search_algorithm_name}_{cost_function_name}_average.png")
            plt.clf()
            plt.cla()
            plt.close()

            print(f"{search_algorithm_name}, {cost_function_name}, {DIMENSIONS}D: min={minimum}, max={maximum}, mean={mean}, median={median}, stddev={stddev}")

        # comparison
        init_graph(cost_function_name, f"{cost_function_name}, {DIMENSIONS}D, average best run comparison")
        ...
        plt.legend()
        plt.savefig(f"{OUTPUT_DIR}{cost_function_name}_comparison.png")
        plt.clf()
        plt.cla()
        plt.close()
